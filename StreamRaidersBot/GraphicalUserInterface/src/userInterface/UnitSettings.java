...
public class UnitSettings {
	...
	public void open(GUI parent) {
		...
		for(UnitRarity ur : UnitRarity.values()) {
			String sur = ur.toString();
			for(String type : utypes.keySet()) {
				JsonObject ut = utypes.getAsJsonObject(type);
				if(ut.get("rarity").getAsString().equals(sur))
					addRow(type, ut.get("name").getAsString(), gui, g++);
			}
		}
		
	}
	
	private void addRow(final String type, final String name, final GUI gui, final int g) {
		...
		String dem = ConfigsV2.getUnitString(cid, lay, type, ConfigsV2.dunEpicMode);
		...
		for(final String s : dunEpicModes) {
			...
			if(m == c) {
				bdem.setGradient(Fonts.getGradient("stngs units buttons on"));
				bdem.setForeground(Fonts.getColor("stngs units buttons on"));
			} else if(m == 0) {
				bdem.setGradient(Fonts.getGradient("stngs units buttons def"));
				bdem.setForeground(Fonts.getColor("stngs units buttons def"));
			} else {
				bdem.setGradient(Fonts.getGradient("stngs units buttons cat"));
				bdem.setForeground(Fonts.getColor("stngs units buttons cat"));
			}
			bdem.setAL(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					ConfigsV2.setUnitString(cid, lay, type, ConfigsV2.dunEpicMode, s);
					for(final String o : dunEpicModes) {
						GUI.setGradient(uid+type+"::dem::"+o, Fonts.getGradient("stngs units buttons def"));
						GUI.setForeground(uid+type+"::dem::"+o, Fonts.getColor("stngs units buttons def"));
					}
					GUI.setGradient(uid+type+"::dem::"+s, Fonts.getGradient("stngs units buttons on"));
					GUI.setForeground(uid+type+"::dem::"+s, Fonts.getColor("stngs units buttons on"));
				}
			});
			gui.addBut(bdem, uid+type+"::dem::"+s);
		}
