...
public class MapGUI {
	...
	public static void showLastHeatMap(Heatmap heatMap, String name, int[] h) {
		...
		GUI map = new GUI("Heatmap " + name, 1000, 800);
	...
	public static void asGui_old(Run run, int slot) {
		Map map = run.getMap(slot);
		if(map == null)
			return;
		
		GUI gui = new GUI("Map " + map.getName(), 1000, 800);
		...
		gui.setGlobalKeyLis(new KeyListener() {
			...
			@Override
			public void keyPressed(KeyEvent e) {
				if((e.getModifiersEx() & KeyEvent.SHIFT_DOWN_MASK) > 0 && (e.getModifiersEx() & KeyEvent.ALT_DOWN_MASK) > 0) {
					Heatmap hm = new Heatmap();
					showLastHeatMap(hm, map.getName(), hm.getMaxHeat(map));
	...
	public static void asGui(run.Run run, int slot) {
